...
module.exports ={
    modificar:(datos,callBack)=>{
        coneccion.query(
            'UPDATE PEDIDOS set cantidad=?,fecha_envio=? where id=?',
            [datos.cantidad,datos.fecha_envio,datos.id],
            (error, results) => {
    ...
    agregar:(datos,callBack) =>{
        coneccion.query(
            'INSERT INTO PEDIDOS (cantidad,estado,user_id,plato_id) values (1,"carrito",?,?)',
    ...
    listar:(callBack) =>{
        coneccion.query(
            'SELECT PED.*,PLA.nom_plato,PLA.precio FROM pedidos PED INNER JOIN platos PLA ON PED.plato_id=PLA.id INNER JOIN usuarios USR ON PED.user_id=USR.id where USR.id=6',
